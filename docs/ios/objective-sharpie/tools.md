...
|Tool|Description|
|--- |--- |
|**bind**|Parses a native framework (`.framework` bundle) or a header file (`*.h`) for an Objective-C library and generates the initial [ApiDefinition.cs and StructsAndEnums.cs](~/ios/objective-sharpie/platform/api-definition-structs-enums.md) files.|
