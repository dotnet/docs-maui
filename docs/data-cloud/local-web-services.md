...
.NET Multi-platform App UI (.NET MAUI) apps that run on Windows or MacCatalyst can consume ASP.NET Core web services that are running locally, and exposed over HTTP or HTTPS, without any additional work. However, additional work is required for an app running in the Android emulator or iOS simulator to consume a local web service, and the process is different depending on whether the web service is running over HTTP or HTTPS.

## Local machine address

The Android emulator and iOS simulator both provide access web services running over HTTP or HTTPS on your local machine. However, the local machine address is different for each.
...
The iOS simulator uses the host machine network. Therefore, apps running in the simulator can connect to web services running on your local machine via the machines IP address or via the `localhost` hostname. For example, given a local secure web service that exposes a GET operation via the `/api/todoitems/` relative URI, an app running on the iOS simulator can consume the operation by sending a GET request to `https://localhost:<port>/api/todoitems/`.

> [!NOTE]
> When running a mobile app in the iOS simulator from Windows, the app is displayed in the [remote iOS simulator for Windows](~/ios/remote-simulator.md). However, the app is running on the paired Mac. Therefore, there's no localhost access to a web service running in Windows for an iOS app running on a Mac.

## Local web services running over HTTP

.NET MAUI apps running in the Android emulator or iOS simulator can consume ASP.NET Core web services that are running locally, and exposed over HTTP, by configuring your .NET MAUI app project and your ASP.NET Core web service to allow clear-text HTTP traffic.

In your code that defines the URL of your local web service, ensure that web service URL specifies the HTTP scheme, and the correct hostname. The `DeviceInfo` class can be used to detect the platform the app is running on. The correct hostname can then be set as follows:
...
In addition, you must add the required network security configuration on Android, and opt-out of Apple Transport Security (ATS) on iOS. For more information, see [Android network security configuration](#android-network-security-configuration) and [iOS ATS configuration](#ios-ats-configuration).

You must ensure also ensure that your ASP.NET Core web service is configured to allow HTTP traffic. This can be achieved by adding a HTTP profile to the `profiles` section of *launchSettings.json* in your ASP.NET Core web service:
...
Then, when launching your web service locally you should run the `http` profile.

### Android network security configuration

To enable clear-text local traffic on Android you should create a network security configuration by adding a new XML file named **network_security_config.xml** in the *Platforms\Android\Resources\xml* folder in your .NET MAUI app project. The XML file should specify the following configuration:
...
To enable clear-text local traffic on iOS you should opt-out of Apple Transport Security (ATS) by adding the following configuration in the *Platforms\iOS\Info.plist* file in your .NET MAUI app project:
...
.NET MAUI apps running in the Android emulator or iOS simulator can consume ASP.NET Core web services that are running locally, and exposed over HTTPS, as follows:
...
In your code that defines the URL of your local web service, ensure that web service URL specifies the HTTPS scheme, and the correct hostname. The `DeviceInfo` class can be used to detect the platform the app is running on. The correct hostname can then be set as follows:
...
This can be accomplished by passing a modified version of the native `HttpMessageHandler` classes to the `HttpClient` constructor, which instruct the `HttpClient` class to trust localhost communication over HTTPS. The `HttpMessageHandler` class is an abstract class, whose implementation on Android is provided by the `AndroidMessageHandler` class, and whose implementation on iOS is provided by the `NSUrlSessionHandler` class.

The following example shows a class that modifies the `AndroidMessageHandler` class on Android and the `NSUrlSessionHandler` class on iOS:
...
.NET MAUI apps running in the Android emulator or iOS simulator can then consume ASP.NET Core web services that are running locally over HTTPS.
