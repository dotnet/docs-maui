...
> [!IMPORTANT]
> A `WebView` must specify its `HeightRequest` and `WidthRequest` properties when contained in a <xref:Microsoft.Maui.Controls.HorizontalStackLayout>, `StackLayout`, or `VerticalStackLayout`. If you fail to specify these properties, the `WebView` will not render.
