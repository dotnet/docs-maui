...
Instances of these brush types can be assigned to the `Stroke` and `Fill` properties of a <xref:Microsoft.Maui.Controls.Shapes.Shape>, the `Stoke` property of a <xref:Microsoft.Maui.Controls.Border>, the `Brush` property of a `Shadow`, and the `Background` property of a <xref:Microsoft.Maui.Controls.VisualElement>.
