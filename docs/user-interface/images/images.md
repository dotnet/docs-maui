...
Devices have a range of screen sizes and densities and each platform has functionality for displaying density-dependent images. To enable .NET MAUI to resize images, you must set the `BaseSize` attribute on each image to a value that's divisible by 8:
...
At build time, images will be resized to the correct resolutions for the target platform and device. The resulting images are then added to your app package.
