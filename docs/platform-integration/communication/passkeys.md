...
> [!IMPORTANT]
> Cancellation doesn't reliably mean the user made a deliberate choice. On **Android**, a missing credential arrives as an <xref:System.InvalidOperationException> instead of a cancellation, so you can tell it apart from the user dismissing the prompt. You can't tell it apart from a configuration or native failure, because Android reports every non-cancellation error with that same type. On **iOS and Mac Catalyst** you can't distinguish it at all: the OS reports a missing credential and a dismissal with the same `ASAuthorizationError.Canceled`, so both arrive as a cancellation. On **Windows** the Security dialog is always shown, so a user with no passkey typically dismisses it, which is also a cancellation. Write your cancellation handler so it's correct in either case.
