...
In this step of the tutorial you'll learn how to use SQLite to store the user's notes.

Currently, the notes are saved as individual text files on the device. Using a database provides the following benefits:

- Storage of all the notes in a single file.
- A query language can be used to find, sort, and filter the data.
- Many different cloud services allow you to centralize a database.
- It's easier to backup a single file rather than multiple files.

## Add the database library

.NET doesn't have built-in support for working with an SQLite database, so you'll need to add a NuGet package to your app to enable SQLite.
