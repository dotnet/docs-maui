...
- The `<UseMaui>true</UseMaui>` project property no longer automatically includes required NuGet packages when [Central Package Management (CPM)](https://learn.microsoft.com/nuget/consume-packages/central-package-management) is enabled.
...
When using [Central Package Management (CPM)](https://learn.microsoft.com/nuget/consume-packages/central-package-management), MAUI packages are no longer referenced automatically and must be explicitly added. For projects that only contain assets (`MauiImage`, `MauiAsset`, etc.), it is sufficient to add the `Microsoft.Maui.Resizetizer` package (instead of `Microsoft.Maui.Controls`).
