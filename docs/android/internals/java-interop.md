...
Managed assemblies, including `Mono.Android.dll`, that contain Java types need to be usable with dynamic registration and with marshal methods. However, both of these approaches have different requirements. It can't be assumed that every assembly will have marshal methods compliant code. Therefore, an approach is required that ensures that code meets this requirement.
