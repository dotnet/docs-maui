...
In this example, the `HslColorViewModel` class defines `Hue`, `Saturation`, `Luminosity`, `Color`, and `Name` properties. When any one of the three color components changes value, the `Color` property is recalculated, and `PropertyChanged` events are fired for all four properties. When the `Color` property changes, the static `GetNearestColorName` method in the `NamedColor` class obtains the closest named color and sets the `Name` property.
