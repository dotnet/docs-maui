...
In the diagram, the gray oval indicates that the app isn't loaded into memory. The light blue ovals indicate that the app is in memory. The solid arcs indicate events that are fired by .NET MAUI, that provide notifications to the running app.
