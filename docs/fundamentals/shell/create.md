...
1. <xref:Microsoft.Maui.Controls.FlyoutItem> or `TabBar`. A <xref:Microsoft.Maui.Controls.FlyoutItem> represents one or more items in the flyout, and should be used when the navigation pattern for the app requires a flyout. A `TabBar` represents the bottom tab bar, and should be used when the navigation pattern for the app begins with bottom tabs and doesn't require a flyout. Every <xref:Microsoft.Maui.Controls.FlyoutItem> object or `TabBar` object is a child of the <xref:Microsoft.Maui.Controls.Shell> object.
1. `Tab`, which represents grouped content, navigable by bottom tabs. Every `Tab` object is a child of a <xref:Microsoft.Maui.Controls.FlyoutItem> object or `TabBar` object.
