...
In addition, adding children to a layout in code in Xamarin.Forms was accomplished by adding the children to the layout's `Children` collection:
...
> [!IMPORTANT]
> Any `Add` layout extension methods, such as <xref:Microsoft.Maui.Controls.GridExtensions.Add%2A> are invoked on the layout, rather than the layouts <xref:Microsoft.Maui.Controls.Layout.Children> collection.
