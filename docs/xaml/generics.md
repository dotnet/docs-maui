...
> [!NOTE]
> The `x:Type` markup extension supplies a Common Language Runtime (CLR) type reference for a generic type, and has a similar function to the `typeof` operator in C#. <!-- For more information, see [x:Type markup extension](~/xamarin-forms/xaml/markup-extensions/consuming.md#xtype-markup-extension). -->
